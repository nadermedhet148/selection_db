  ...
  <div>
    <v-card :loading="searchLoading" :disabled="searchLoading">
      ...
      <v-card-text>
        <v-row>
          <template
            v-for="(h, i) in newCommersTable.headers.filter(h => h.inSearch)"
          >
      ...
      <v-card-actions class="px-4 py-4">
        <v-btn
          class="px-6"
          :v-if="newCommersTable.items.length > 0"
          @click="findItems()"
          large
          color="primary"
          v-text="'بحث'"
    ...
    >
      ...
      >
        ...
        <v-card-text>
          <v-row>
            <template
              v-for="(h, i) in newCommersTable.headers
                .filter(h => h)
    ...
    async calculateRecRegionWithKnowloadage() {
      ...
      this.$set(
        ...
        Object.keys(groupdWithRecRegion).map(key => ({
          RecuRegion: key,
          High: groupdWithRecRegion[key].filter(
            ele => ele.KnowledgeLevel == constants.levelMappingEn.High
          ).length,
          AboveAvg: groupdWithRecRegion[key].filter(
            ele => ele.KnowledgeLevel == constants.levelMappingEn.AboveAvg
          ).length,
          Avg: groupdWithRecRegion[key].filter(
            ele => ele.KnowledgeLevel == constants.levelMappingEn.Avg
          ).length,
          Normal: groupdWithRecRegion[key].filter(
