    ...
    <div v-if="!passwordDialog.model">
      <v-card :loading="searchLoading" :disabled="searchLoading">
        ...
        <v-card-text>
          <v-row>
            <template
              v-for="(h, i) in recommandationsTable.headers.filter(
                h => h.inSearch
              )"
            >
        ...
        <v-card-actions class="px-4 py-4">
          ...
          <v-btn
            ...
            @click="calculateUnitWithCertificatorsStats()"
            large
            color="primary"
            v-text="'يومية عددية بالمصدقين علي الوحدات'"
          ></v-btn>
          <v-btn
            class="px-6"
            v-if="recommandationsTable.items.length > 0"
            @click="calculateRecommandtionsWithCity()"
            large
            color="primary"
      ...
      <v-dialog
        persistent
        v-model="unitWithCertificatorsStatsTabel.isDisplayed"
        scrollable
      >
        <v-card>
          <v-btn
            @click="unitWithCertificatorsStatsTabel.isDisplayed = false"
            icon
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
          <table-bulider
            :headers="unitWithCertificatorsStatsTabel.headers"
            :printer="unitWithCertificatorsStatsTabel.printer"
            :items="unitWithCertificatorsStatsTabel.items"
            :title="'يومية عددية بالمصدقين علي الوحدات'"
          >
          </table-bulider>
        </v-card>
      </v-dialog>
      ...
      <v-dialog
        persistent
        v-model="cityWithRecommandationsTable.isDisplayed"
        scrollable
      >
        <v-card>
          <v-btn @click="cityWithRecommandationsTable.isDisplayed = false" icon>
            <v-icon>mdi-close</v-icon>
          </v-btn>
          <table-bulider
            :headers="cityWithRecommandationsTable.headers"
      ...
      >
        ...
        >
          ...
          <v-card-text>
            <v-row>
              <template
                v-for="(h, i) in recommandationsTable.headers
                  .filter(h => h.inModel)
...
export default {
  ...
  methods: {
    changeMatching() {
      if (
        this.Recommandation.soldierDirection &&
        this.Recommandation.UnitDirection
      ) {
        this.$set(
          this.Recommandation,
          "Matching",
    ...
    async calculateRecommandtionsWithCity() {
      ...
      const solidersCount = await this.api(`global/queryRunners`, {
        query: `
              select count(Soldier.ID) as total , City .City , Soldier.KnowledgeLevel, Soldier.RecuStage from Soldier
              join City on Soldier.CityID = City.CityID 
              where Soldier.RecuStage = N'${this.search.RecuStage}'
              GROUP by City .City , Soldier.KnowledgeLevel , Soldier.RecuStage
              `
      });
      ...
      const directionWithRecommandationsItems = solidersCount.data.map(ele => ({
        ...
        matched: recommandations.data.filter(
          recommandation =>
            ...
            recommandation.Soldier?.City.City === ele.City &&
      ...
      this.$set(this.cityWithRecommandationsTable, "isDisplayed", true);
      this.$set(
    ...
    calculateUnitWithCertificatorsStats() {
      ...
      const groupdWithCertificator = _.groupBy(
      ...
      this.$set(
        this.unitWithCertificatorsStatsTabel,
        "items",
        Object.keys(groupdWithUnit).map(Unit => {
          const unitStats = {
            Unit
          };
          Object.keys(groupdWithCertificator).forEach(key => {
            unitStats[key] = groupdWithUnit[Unit].filter(
              ele => ele.Certificator == key
            ).length;
          });
      ...
      this.$set(this.unitWithCertificatorsStatsTabel, "printer", {
        ...
        excelKey: "data",
        excelHeaders: this.unitWithCertificatorsStatsTabel.headers.filter(
          f => f.inTable
        )
      });
    },
