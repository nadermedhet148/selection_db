<template>
  <div>
          ...
          <v-row>
            ...
            >
              <v-col
                :key="i"
                cols="12"
                :md="col.fullwidth ? '12' : mokataba.fullscreen ? '3' : '6'"
              >
                <v-text-field
            ...
            <v-col cols="12" class="mt-0 pt-0">
              ...
              >
                <template
                  v-for="(wared, wi) in mokataba.item.connectedMokatabas.split(
                    ','
                  )"
                >
              ...
              <v-chip @click="wared.model = true" outlined color="primary">
                إضافة وارد
              </v-chip>
            </v-col>
          </v-row>
    ...
    <v-dialog
      ...
      v-model="wared.model"
      max-width="1000"
      :fullscreen="wared.fullscreen"
    >
      <v-card>
        <v-card-title>
          اختر مكاتبة من الوارد
          <v-spacer></v-spacer>
          <v-btn icon @click="wared.fullscreen = !wared.fullscreen">
            <v-icon
              >mdi-window-{{
                wared.fullscreen ? "restore" : "maximize"
              }}</v-icon
            >
          </v-btn>
          <v-btn icon @click="wared.model = false">
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>
        <v-divider></v-divider>
        <v-data-table
          fixed-header
          :headers="
            wared.headers.filter(
              f => (f.adminOnly && isAdmin()) || !f.adminOnly
            )
          "
          :items="wared.items"
          :search="wared.search"
          v-model="wared.selected"
          hide-default-header
        >
          <template v-slot:header="table">
            <table-header-filters
              :items="wared.items"
              :table="table"
              :filters.sync="wared.tableFilters"
            ></table-header-filters>
          </template>
          <template v-slot:footer="table">
            <table-footer-filters
          ...
          <template v-slot:item.actions="{ item }">
            <v-btn
              icon
              @click="deselectWared(item.number)"
              v-if="
                mokataba &&
                  mokataba.item &&
            ...
            <v-btn v-else icon @click="selectWared(item.number)">
  ...
  methods: {
    ...
    async getOldSendTo() {
      ...
      if (items.ok && items.data) {
        ...
        this.$set(this.selects, "sendTo", [...new Set(mapped)]);
      }
    },
    async addOrUpdate() {
    ...
    async getMokatabas() {
      ...
      this.$set(this.searched, "items", sader);
      this.$set(this.searched, "printer", printer);
      this.$set(this.wared, "items", wared);
    }
  }
};
</script>
