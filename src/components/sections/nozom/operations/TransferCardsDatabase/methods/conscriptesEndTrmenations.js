module.exports = $this => {
  $this
    .api(
      "global/get_all",
      {
        table: "conscriptesEndTrmenations",
    ...
    .then(async x => {
      let data = x.data,
        ...
        idMap = {
          ...
          trmenationId: {
            key: "stateId",
            map: $this.loadIdMap("endtrementations")
          }
        };
        ...
        return new Promise((success, failure) => {
          (async function loop() {
            ...
            for (let i = 0; i < conscriptes.length; i++) {
              /*[i].militaryId = conscriptes[i].militaryId.replace(
                /\D+/g,
                ""
              );*/
              if (
    ...
    .catch(error => {
      console.log(error);
      $this.showError(
        "An error occured while trying to get data from conscriptesEndTrmentations table"
      );
    })
    .finally(() => {
      $this.resetProgress();
