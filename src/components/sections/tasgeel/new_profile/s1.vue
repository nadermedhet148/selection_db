<template>
  <div>
    <v-card class="mb-6" tile flat>
          ...
          <v-col class="text-center align-self-end">
            <v-menu transition="slide-y-transition">
              <template v-slot:activator="{ on }">
                <v-btn v-on="on">
                  عرض بيانات
                  {{ c.isWeb ? "الشبكة العسكرية" : "فرع السجلات" }}
      ...
      <v-card-text v-if="c.militaryOperationParticipants.length">
        <v-divider class="mx-4"></v-divider>

        <v-card-title>العمليات الحربية </v-card-title>
        <v-chip-group
          v-model="selection"
          active-class="deep-purple accent-4 white--text"
          column
        >
          <template v-for="(ele, i) in c.militaryOperationParticipants">
    ...
    <template v-for="(card, ci) in ccards">
      <v-card
        v-if="!card.typeId || card.typeId == c.typeId"
        class="mb-6"
        :key="ci"
      >
        <v-card-title class="mb-0 color_moqadem white--text">
        ...
        >
          <template v-for="(row, i) in card.rows">
            <v-row class="my-1" :key="i">
              <template v-for="(f, ii) in row">
                <v-col
                  v-if="
                    f.hasOwnProperty('key') &&
                      (!f.notInServiceOrEhtyat ||
                        (f.notInServiceOrEhtyat && c.stateId !== 101))
                  "
                  :key="'k_' + ii"
                ...
                <v-col
                  ...
                  v-if="
                    (f.hasOwnProperty('val') || f.hasOwnProperty('localVal')) &&
                      (!f.notInServiceOrEhtyat ||
                        (f.notInServiceOrEhtyat && c.stateId !== 101))
                  "
                  :key="'v_' + ii"
                ...
                >
                  ...
                  >
                    ...
                    >
                      <div style="width: 100%">
                        <template
                          v-if="cc_types[f.lvkey] == 'prevPeriodCombination'"
                        >
                          <v-row class="mt-2 mr-2">
                            <v-col>
                              <v-text-field
                                v-model="cc['prevPeriod'].days"
                                :rules="ruleDays"
                                filled
                                type="number"
                                label="يوم"
...
export default {
  ...
  methods: {
    ...
    async saveEverything() {
      ...
      let updatePrevPeriod = await this.api(
        "global/" + (data.prevPeriod.id ? "update" : "create") + "_one",
        {
          table: "prevPeriod",
          where: {
            militaryId
          ...
          update: {
            years: data.prevPeriod.years,
            months: data.prevPeriod.months,
            days: data.prevPeriod.days,
            militaryId
          }
        }
      );
      ...
      this.addModification(militaryId, 2, 9);
    ...
    onlyLocalPeriod() {
      this.cards[5].rows.forEach(arr => {
        arr[0].val = arr[0].localVal;
      });
    }
  },
  computed: {
    ccards() {
      // this.onlyLocalPeriod();
          ...
          row.forEach((f, ii) => {
            if (f.val) {
              ...
              if (
                f.type == "split" &&
                (!f.notInServiceOrEhtyat ||
                  (f.notInServiceOrEhtyat && this.c.stateId !== 101))
              ) {
  ...
  data: () => ({
    imageLoading: false,
    ruleDays: [
      v => !isNaN(v) || "يجب إدخال ارقام فقط",
      v => v == null || v <= 29 || "لا يمكن تخطي 29 يوم"
    ],
    ruleMonths: [
      v => !isNaN(v) || "يجب إدخال ارقام فقط",
      v => v == null || v <= 11 || "لا يمكن تخطي 11 شهر"
    ],
    ruleYears: [v => !isNaN(v) || "يجب إدخال ارقام فقط"],
    cc: {},
    highlights: ["demobilizationDate"],
    selects: {
      areaId: {
    ...
    cc_types: {
      ...
      notes: "textarea",
      notesDemobilization: "textarea",
      degreeId: "select",
      unitId: "select",
      // demobilizationDate: "date",
      prevPeriodCombination: "prevPeriodCombination" // special case
    },
    isWebTypes: [
      {
        title: "عرض بيانات الشبكة العسكرية",
        value: true
      },
      {
        title: "عرض بيانات فرع السجلات",
        value: false
      }
    ],
    cards: [
      {
        ...
        rows: [
          ...
          [
            {
              key: "الدرجة",
              val: "webPromotions.0.webDegree.displayedText",
              localVal: "degree.degreeType",
              dbkey: "degreeId"
            },
            {
              key: "الوحدة الحالية",
              val: "webUnits.0.webUnit.displayedText",
              localVal: "unit.unitText",
              dbkey: "unitId"
            }
          ],
          [
          ...
          [
            {
              key: "حالة محو الأمية",
              val: "ignorant.displayedText",
              localVal: "ignorant.displayedText",
              dbkey: "ignorantId"
            },
            {
              key: "مؤيد محو الأمية",
              val: "ignorantSupporter.displayedText",
              localVal: "ignorantSupporter.displayedText",
              dbkey: "ignorantSupporterId"
            }
          ],
          [
            {
      ...
      },
      {
        ...
        rows: [
          ...
          [
            {
              key: "مدة سابقة حسنة",
              localVal: "prevPeriodCombination",
              type: "split",
              splitWith: ["-", "/"]
            }
          ],
          ...
          [
            {
              key: "تاريخ إنهاء الخدمة ( حالة إنهاء الخدمة لأسباب )",
              val: "webDemobilizationDate",
              localVal: "endingDutyDate",
              type: "split",
              splitWith: ["-", "/"],
              notInServiceOrEhtyat: true
            }
          ]
          // [
          //   {
      ...
      // },
      {
        ...
        rows: [
          ...
          [
            ...
            {
              val: "notesDemobilization",
              localVal: "notesDemobilization"
            }
          ]
        ]
      }
