<template>
  <div>
    <v-card class="mb-6">
      <v-card-title>
        البحث المتقدم عن التحقيقات / المحاكم
      </v-card-title>
      <v-card-text>
...
export default {
  ...
  methods: {
    searchIgnorants() {
      ...
      this.api("sections/tasgeel/search/courts", {
        search
      })
        .then(x => {
          console.log(x);
